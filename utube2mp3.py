...
def update_packages():
  updated_programs = ''
  update_log = sp.Popen(["pip list --outdated"], shell=True, stdout=sp.PIPE)
...
def url_to_list(link_string: str):
  ...
  escaped_string = (link_string.replace('\r', '\\r').replace('\n',
                                                             '\\n').replace(
                                                               '\t', '\\t'))
...
def upload_to_dropbox(dbx_account: Type[dropbox.Dropbox], mp3_files: list,
                      remoteDir: str):
  for music in mp3_files:
    ...
    with open(music, 'rb') as music_file:
      try:
        dbx_account.files_upload(music_file.read(), remoteDir + "/" + music)
  ...
  while resume:
    ...
    while not conversion_ready:
      comp_or_dropbox = input("Would you like to save the files to "
                              ...
                              " local directory) \n").lower()
      if comp_or_dropbox in ('d', "dropbox", "drop"):
        mp3_to_dropbox = True
        retype_token = True
        ...
        while retype_token:
          ...
          while not valid_dropbox_directory:
            ...
            path_exists = check_dropbox_path(dbx, dbx_directory, local_inf)
      ...
      elif comp_or_dropbox in ('l', "local", "localdirectory",
                               "local directory"):
        while True:
    ...
    if mp3_to_dropbox:
      ...
      upload_to_dropbox(dbx, mp3_list, dbx_directory)
      print("----------------------------------------------------------"
            "------------------------------------")
